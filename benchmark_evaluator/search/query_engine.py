...
def get_search_results_with_facets(query, query_id, conn, facet_list, use_facets_as_filters=True):
    """
    Retrieve both documents and facets given a query
    """
    query_with_results = SearchResults(query_text=query, query_id=query_id)
    search_result = __es_search(query, conn, facet_list=facet_list, use_facets_as_filters=use_facets_as_filters)

    documents_results = __get_documents(search_result)
    if documents_results and len(documents_results) > 0:
        ...
        facets = __get_facets(search_result)
...
def __es_search(query, conn, facet_list=None, topic_similarity_threshold=0.5, use_facets_as_filters=True):
    ...
    query_with_params = __append_common_params(conn, query, topic_similarity_threshold,
    ...
    return __query_service(conn.get_search_service_url(), query_with_params, conn)
